package blog.thoughts.on.java.forge.view;
...
public class BookBean implements Serializable
{
   ...
   public String delete()
   {
      ...
      try
      {
         ...
         Author author = deletableEntity.getAuthor();
         author.getBooks().remove(deletableEntity);
         deletableEntity.setAuthor(null);
   ...
   private Predicate[] getSearchPredicates(Root<Book> root)
   {
      ...
      int pages = this.example.getPages();
      if (pages != 0)
      {
         predicatesList.add(builder.equal(root.get("pages"), pages));
      }
